...
ZSSEditor.getJoinedFocusedFieldIdAndCaretArguments = function() {
    ...
    var field = ZSSEditor.getFocusedField();
    var id;
    if(field) {
        id = field.getNodeId();
    }
    else {
        id = 'zss_field_content';
    }
    var idArgument = "id=" + id;
...
function removeFomat() {
    try {
        // 如果没有, 则到cursor所在的span或p下
        var selection = window.getSelection();
        var range = selection.getRangeAt(0).cloneRange();
        ...
        if(nodeName != "body" && nodeName != "html") {
            if(nodeName == 'div' && curNode.className == 'zss_field_content') {
                return;
            }
            var replaceNodeName = nodeName;
            // 如果是heading则替换之
            if(nodeName.indexOf('h') != -1 || nodeName == 'blockquote') {
                replaceNodeName = 'p';
            }
            else if(nodeName === 'a') {
                replaceNodeName = 'span';
            }

            var tokenId = 'LEACLR-' + (new Date()).getTime();
            var t = '~*Z+&';
            var tSpan = '<span id="' + tokenId + '"></span>';
            var tokenSpan = $(tSpan).get(0);
            var replacedText;
            if($node.find('img').length > 0) {
                range.insertNode(tokenSpan);
                replacedText = $node.html();
            }
            else {
                tokenSpan.innerHTML = t;
                range.insertNode(tokenSpan);
                replacedText = $node.text();
                replacedText = replacedText.replace(t, tSpan);
            }

            // 替换之
            $node.replaceWith('<' + replaceNodeName + '>' + replacedText + '</' + replaceNodeName + '>')
            // move cursor到span下
            var range = document.createRange();
            var sel = window.getSelection();
            range.setStart($('#' + tokenId).get(0), 0);
            range.collapse(true);
            sel.removeAllRanges();
            sel.addRange(range);
            // 可能有问题
...
ZSSEditor.insertImage = function(urls, alt) {
    ZSSEditor.restoreRange();
...
ZSSEditor.sendEnabledStyles = function(e) {
    ...
    if (!focusedField) {
        focusedField = ZSSEditor.editableFields['zss_field_content'];
    }
